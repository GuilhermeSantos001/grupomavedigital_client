...
export function usePeoplePHService(take: number = 10) {
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [uri, setURI] = useState<string>(`${process.env.NEXT_PUBLIC_API_HOST}/php?take=${take}`);
  const [lastCursorId, setLastCursorId] = useState<number>(0);

  const skip = 1;

  const { data, error, mutate } = useSWR<
    ApiResponseSuccessType<PersonPHType[]>,
    ...
    return {
      ...
      nextPage: () => {
        if (data.data.length !== 0) {
          ...
          setURI(`${process.env.NEXT_PUBLIC_API_HOST}/php?skip=${skip}&take=${take}&cursorId=${cursorId}`);
        }
      },
      previousPage: () => {
        ...
        setURI(`${process.env.NEXT_PUBLIC_API_HOST}/php${query}`);
      },
      refreshPage: async () => {
      ...
      update: async (id: string, newData: DataPersonPH): Promise<boolean> => {
        const uri = `${process.env.NEXT_PUBLIC_API_HOST}/ph/people/${id}`;
        ...
        if (!updateData.success) {
          ...
          mutate({
            success: true,
            data: data.data.map(personPH => {
              if (personPH.id === id) {
                personPH = { ...personPH, ...updateData.data };
              }
      ...
      delete: async (id: string): Promise<boolean> => {
        ...
        if (!deleteData.success) {
          ...
          mutate({
            success: true,
            data: data.data.filter(personPH => personPH.id !== id)
          });
        }
