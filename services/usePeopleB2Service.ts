...
export function usePeopleB2Service(take: number = 10) {
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [uri, setURI] = useState<string>(`${process.env.NEXT_PUBLIC_API_HOST}/b2p?take=${take}`);
  const [lastCursorId, setLastCursorId] = useState<number>(0);

  const skip = 1;

  const { data, error, mutate } = useSWR<
    ApiResponseSuccessType<PersonB2Type[]>,
    ...
    return {
      ...
      nextPage: () => {
        if (data.data.length !== 0) {
          ...
          setURI(`${process.env.NEXT_PUBLIC_API_HOST}/b2p?skip=${skip}&take=${take}&cursorId=${cursorId}`);
        }
      },
      previousPage: () => {
      ...
      update: async (id: string, newData: DataPersonB2): Promise<boolean> => {
        ...
        if (!updateData.success) {
          ...
          mutate({
            success: true,
            data: data.data.map(personB2 => {
              if (personB2.id === id) {
                personB2 = { ...personB2, ...updateData.data };
              }
      ...
      delete: async (id: string): Promise<boolean> => {
        ...
        if (!deleteData.success) {
          ...
          mutate({
            success: true,
            data: data.data.filter(personB2 => personB2.id !== id)
          });
        }
