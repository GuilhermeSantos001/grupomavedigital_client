...
const serverSideProps: PageProps = {
  ...
  description: 'Administração dos recebimentos dos funcionarios(as) de Pacote de Horas',
  themeColor: '#004a6e',
      ...
      <div className="col-12">
        <div className="p-3 bg-primary bg-gradient rounded">
          <p className="text-center text-secondary fw-bold fs-5 my-2">
            ...
            /> Histórico de Pagamento (Pacote de Horas)
          </p>
        </div>
        <button
          type="button"
        ...
        <ListWithCheckbox
          title='Pagamento em dinheiro (Pacote de Horas)'
          messages={{
            ...
            if (postings.length > 0) {
              ...
              for (const posting of postings) {
                rows.push({
                  ...
                  originDate: `${new Date(posting.contractStartedAt).toLocaleDateString()} A ${new Date(posting.contractFinishAt).toLocaleDateString()}`,
                  matricule: posting.personPH.person.matricule,
                  personCoverage: posting.personPH.person.name,
                  reasonForAbsence: 'Pacote de Horas',
              ...
              SheetEx<LayoutCashierPay>(rows, {
                sheetName: `Pagamentos em Dinheiro Pacote de Horas - ${costCenters.find(costCenter => costCenter.id === costCenterId)?.value || '???'} (${periodStart.toLocaleDateString()} - ${periodEnd.toLocaleDateString()})`,
