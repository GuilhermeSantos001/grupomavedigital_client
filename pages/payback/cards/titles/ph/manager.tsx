  ...
  return (
    <div className="row g-2">
      <div className="col-12">
        ...
        <ListWithCheckbox
          title='Pagamento Cartão Alelo (Pacote de Horas)'
          messages={{
          ...
          onClick={() => {
            if (postingsFiltered.length > 0) {
              ...
              for (const posting of postingsFiltered) {
                if (posting.personPH.person.cards[0].unlocked && posting.personPH.person.cards[0].status === 'available') {
                  rows.push({
                    serialNumber: posting.personPH.person.cards[0].serialNumber,
                    cpf: posting.personPH.person.cpf,
                    value: posting.paymentValue.toString().replace('.', ','),
                    description: posting.description || `Crédito referente ao pagamento de Pacote de Horas do periodo ${periodStart.toLocaleDateString()} a ${periodEnd.toLocaleDateString()}`,
                  })
                } else {
                  errors.push(`O cartão do(a) ${posting.personPH.person.name} não está desbloqueado ou está cancelado.`);
                }
              }
        ...
        <button
          ...
          onClick={async () => {
            ...
            for (const postingId of selectedPostings) {
              ...
              if (
                posting.personPH.person.cards[0].status !== 'available' ||
                !posting.personPH.person.cards[0].unlocked
              )
