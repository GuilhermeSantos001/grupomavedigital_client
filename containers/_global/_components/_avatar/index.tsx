...
export default function Component({ size, hash }: Props) {
  ...
  return (
    loading
      ? <Avatar icon={<Skeleton.Image active style={{ width: size, height: size }} />} size={size} />
      : <Avatar icon={<Image src={!isLogged ? '/logo_circle_dark.svg' : url} alt="MergeOps" preview={false} />} size={size} />
  )
}
